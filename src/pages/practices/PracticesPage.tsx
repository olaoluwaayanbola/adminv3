  ...
  return (
    <section className="space-y-6">
      <header className="space-y-2">
        ...
        <h1 className="text-3xl font-semibold text-cp365-textMain">Practice</h1>
        <p className="text-sm text-cp365-textMuted">List of practice</p>
      </header>
      ...
      <Card className="rounded-3xl bg-white p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-cp365-textMuted">Manage active practices and locations.</p>
          </div>
          ...
          <table className="w-full border-collapse text-left text-sm text-cp365-textMain">
            <thead className="bg-cp365-surface text-xs uppercase tracking-wide text-cp365-textMuted">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold">Practice Code</th>
              ...
              {practices.map((practice) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="font-semibold text-cp365-textMain">{practice.name}</div>
                    <p className="text-xs text-cp365-textMuted">{practice.city}, {practice.state}</p>
                  </td>
      ...
      <Modal
        ...
        footer={
          <Button type="button" variant="primary" onClick={() => setIsInfoModalOpen(false)}>
            Got it
          </Button>
        }
      >
        <p>
          For now, please contact the implementation team to add new practices. This shortcut will
          activate when the workflow is ready.
        </p>
      </Modal>
    </section>
  );
